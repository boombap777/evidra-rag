...
This script queries the knowledge hub with the configured ``retrieval.strategy``:
the benchmark-selected Dense baseline or the optional Dense + BM25 + RRF path.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default=os.getenv(
            "MODULAR_RAG_SETTINGS_PATH",
            str(_REPO_ROOT / "config" / "settings.yaml"),
        ),
        help=(
            "Path to configuration file (default: MODULAR_RAG_SETTINGS_PATH "
...
def _run_query(
    ...
    trace.metadata["retrieval_strategy"] = hybrid_search.config.strategy
...
def main() -> int:
    ...
    try:
        ...
        store = getattr(getattr(hybrid_search, "dense_retriever", None), "vector_store", None)
