...
## 8. 开发者技术知识检索：当前基线与最小收口

本节用于使当前实现、评测报告与项目表述保持一致，不把该 MCP Server
扩展为售后 Agent，也不引入 LangGraph。除明确标记为 Observed 的结果外，新增内容均为
vNext 计划；只有完成对应任务并生成可复现证据后，才能改写为已实现结论。

### 8.1 产品边界

本项目面向开发者查询技术文档、报错说明和解决方案的需求，提供本地优先、可插拔、
可评测的技术知识检索服务，并通过 MCP 向外部 Agent 返回带来源的检索上下文。

- Server 负责文档摄取、索引、检索、引用和 Trace，不负责外部 Agent 的任务规划。
- 当前本地链路不代表生产知识库、真实企业文档规模或线上用户效果。
- 检索指标只评价相关文档排名，不得表述为生成答案准确率。
- Cross-Encoder 是被评测的可选路径，不预设其一定优于 Dense。

### 8.2 Observed 检索基线

权威产物为 `data/benchmarks/stackoverflow/benchmark_report.json`。当前报告基于公开
`mteb/stackoverflowdupquestions-reranking` 数据集、固定种子 `20260901`、500 条查询、
14,613 篇全局语料和 `K=10`：

| Method | HitRate@10 | MRR@10 | P95 latency |
|---|---:|---:|---:|
| BM25 | 0.718 | 0.3984 | 10.07 ms |
| Qwen3 Dense | 0.838 | 0.5295 | 90.48 ms |
| RRF | 0.834 | 0.4810 | 90.06 ms |
| RRF + Cross-Encoder | 0.812 | 0.4914 | 267.66 ms |

Dense 相比 BM25 的 HitRate@10 提升 `12.0` 个百分点，MRR@10 相对提升 `32.9%`；
RRF 与重排路线相对 Dense 的 HitRate@10 分别低 `0.4`、`2.6` 个百分点。延迟来自
100 条预热后的本地查询；它不是并发服务压测或生产 SLA。当前选择 Dense 作为质量
基线，并默认关闭重排。

### 8.3 最小运行时对齐

新增配置项 `retrieval.strategy: dense | hybrid_rrf`，并使 CLI 与
`query_knowledge_hub` 使用同一策略选择逻辑。本地默认值计划设为 `dense`；RRF 和
Cross-Encoder 继续作为可切换实验路径，重排失败时回退到进入重排前的排序结果。

MCP 对外保持三个 Tool：

- `query_knowledge_hub`：检索并返回带引用的相关片段；
- `list_collections`：列出集合及统计信息；
- `get_document_summary`：返回已有摘要或首段内容预览，不暗示实时 LLM 摘要生成。

文档摄取由用户通过 CLI 或 Dashboard 触发
`Load -> Split -> Transform -> Embed -> Upsert` 流水线；在实现目录监听、调度器或
消息队列前，不描述为后台自动发现入库。

### 8.4 测试与证据收口

默认离线测试不得依赖云端凭据或外部网络。Azure/OpenAI/Ollama Live、Ragas 在线评测
和其他可选集成必须显式 opt-in；无凭据时应跳过而非失败。需要修复并回归当前已发现的
Ragas/LangChain 依赖兼容、Windows Chroma 文件句柄、Trace 字段与计时、MCP 参数契约
和测试顺序隔离问题。

成功查询的 Trace 至少记录策略、provider、阶段耗时、候选 ID/分数和最终排名；摄取
Trace 的每个标准阶段必须包含统一的 `method` 字段。后续基准清单还应绑定代码快照、
数据集、Embedding GGUF、llama.cpp、Reranker、硬件、预热与样本数量，延迟波动不得
改变冻结排名和质量指标的判定。

### 8.5 Schedule

- [x] J1 - 已增加 `retrieval.strategy: dense | hybrid_rrf`；CLI、MCP Trace 与 Dashboard
  从同一 Settings 读取策略。本地配置默认 `dense`，Hybrid/Rerank 继续保留为显式可配
  实验路径；`test_j1_retrieval_strategy.py` 证明 Dense 路径不会调用 Sparse/RRF。
- [x] J2 - 已补齐高精度阶段计时、标准摄取阶段 `method`、最终排名 Trace、公开 MCP
  Schema 预校验、Chroma 显式 close/context manager、Ragas 0.4.3 与 LangChain 兼容
  约束，并把真实模型/API测试归为 `--run-live` opt-in。
- [x] J3 - 默认测试使用无凭据、无模型服务的 `local_hash` 合同测试后端；预验收保存
  `test_data/j-pre-final.xml`（1,350 passed、35 个明确 opt-in/可选项 skipped、0 failed）
  与 `coverage-core-pre-final.json`（固定核心范围 84%）。MCP stdio E2E 分别调用
  `query_knowledge_hub`、`list_collections`、`get_document_summary`。
- [x] J4 - `scripts/benchmark_manifest.py` 校验 500 条 golden、14,613 行语料与 500 组
  排名，并绑定源码树、`uv.lock`、报告、语料、向量缓存、BM25、Qwen3 GGUF 和
  llama.cpp 的 SHA-256。当前目录没有 `.git`，因此 manifest 明确记录
  `git_available=false`，不声称 commit；README 已替换为 Observed 口径，并将历史模板
  数字标为非实测。

最终收口由 `scripts/resume_release_gate.py --require-ready` 执行。它要求 benchmark
manifest 与当前源码一致、默认测试零失败、指定合同测试全部出现在同一份 JUnit、固定
核心范围覆盖率不少于 80%，并核对生产配置仍为 Qwen3 Dense 与三个公开 MCP Tool。
输出的三项 claim 只有全部为 `SATISFIED` 时才能写作当前项目事实。证据边界始终是公开
数据集、本地单机和离线/协议测试，不代表生产并发、线上 SLA 或真实企业用户效果。
...
- [x] [PUB1] Prepare the portable campus-project source release; no Git commit or upload.
  ...
  - Scope expanded by owner approval: implement the minimal search workspace described in
    docs/ui-design.md, reuse Streamlit/core retrieval, and verify real query and error states.
    Do not generate answers, add a separate frontend stack, retrain models, or expose a public server.
