...
仓库名与工程目录名：`evidra-rag`。为保持兼容，内部 Python 分发包 `modular-rag-mcp-server`、
导入路径与现有命令行入口不随此次命名变更重写。原实验报告中的历史名称仍保留。

## 无模型快速体验

安装 Python 3.12 和 uv，在仓库根目录执行：
...
打开 http://127.0.0.1:8503/，点击“加载 3 篇内置样例”，搜索 `MCP 工具 边界`。
新首页展示片段、来源和实际耗时；管理页仍在侧边栏。离线演示使用 `local_hash`，
不需要凭据或模型，但**不代表 Qwen3 语义检索质量**。Qwen3 配置保留在
`config/settings.yaml`；模型准备、完整基准与发布范围见 [PUBLICATION.md](PUBLICATION.md)。

> 面向开发者技术文档、报错说明和解决方案查询的本地优先 RAG 服务。系统以 Qwen3 Dense 作为已评测默认检索策略，保留 BM25、RRF 与 Cross-Encoder 实验路径，并通过 MCP 向外部 Agent 提供带来源的检索上下文。
...
本项目实现用户触发的 `Load → Split → Transform → Embed → Upsert` 摄取流水线、可配置检索、引用组装、Trace、离线评测和三个 MCP Tool。它是检索服务，不承担外部 Agent 的任务规划；“文档自动入库”指触发后自动执行整条流水线，不代表后台自动发现文件。
...
| **Retrieval** | 默认 Dense；可选 BM25 + Dense + RRF + Rerank | 默认策略来自冻结基准的质量门禁，实验路径可由 YAML 切换 |
...
**🔍 质量门禁驱动策略选择**：在同一公开数据集上比较 BM25、Qwen3 Dense、RRF 和 Cross-Encoder 重排；由于本次 RRF/重排没有超过 Dense，默认采用 Dense，复杂路径保留为可配置实验能力。
...
**📊 可视化管理 + 自动化评估**：Streamlit Dashboard 展示数据、摄取/查询 Trace 和评估结果；Ragas 在线评测显式 opt-in，默认测试不需要云端凭据或外部网络。
...
当前 E 盘目录是独立源码快照，未包含 `.git`，因此本文档不声称任何分支、commit 数量或提交历史。`data/benchmarks/stackoverflow/benchmark_manifest.json` 记录源码树、`uv.lock`、评测数据、排名、模型与 llama.cpp 运行时的 SHA-256；运行 `python scripts/benchmark_manifest.py verify` 可检查证据是否仍与当前源码一致。

### Observed 检索基线

权威结果为 `data/benchmarks/stackoverflow/benchmark_report.json`。在公开 `mteb/stackoverflowdupquestions-reranking` 测试集的固定种子 `20260901` 下，从 500 条查询构建 14,613 篇全局语料，评价 K=10：
...
Dense 相比 BM25 的 HitRate@10 提升 12.0 个百分点，MRR@10 相对提升 32.9%。延迟来自单机预热后的 100 条顺序查询，不是并发压测或生产 SLA；RRF 和重排在本次冻结数据上未超过 Dense，因此没有被设为默认。
...
> ⚠️ **事实边界**：只有本节“当前仓库可引用版本”和上方 Observed 表中的数字属于当前仓库证据。后面的历史写作示例含虚构业务规模与效果，只展示结构，绝不能作为本项目实测结果。
>
> 简历项目经历需要结合岗位定制，但不得把目标、Fake/离线数据或历史模板数字写成生产效果。
>
...
**开发者技术知识检索与评测 RAG 平台** | 个人项目 | RAG / AI 应用开发

- **检索评测**：基于公开 Stack Overflow 数据整理 500 条查询、14,613 篇全局语料；Qwen3 Dense 取得 HitRate@10 83.8%、MRR@10 0.529，较 BM25 分别提升 12.0 个百分点和相对提升 32.9%。比较 BM25、Dense、RRF 与 Cross-Encoder 后，以质量门禁选择 Dense 为默认方案。
- **架构与部署**：将 LLM、Embedding、Splitter、VectorStore、Reranker、Evaluator 以接口与 Factory 解耦并通过 YAML 切换；使用 llama.cpp + Qwen3-Embedding GGUF 完成本地部署，100 条预热后顺序查询的 Dense P95 为 90.5 ms。
- **MCP 与调试**：暴露 `query_knowledge_hub`、`list_collections`、`get_document_summary` 三个 MCP Tool；用户触发摄取后自动完成 Load/Split/Transform/Embed/Upsert，并记录策略、provider、候选 ID/分数、最终排名和阶段耗时用于分析 Bad Case。

证据入口：`data/benchmarks/stackoverflow/benchmark_report.json`、`benchmark_manifest.json`、`test_data/resume-release-gate.json`。
...
### 历史写作示例一：校招 · RAG Engineer 方向（非实测）
...
### 历史写作示例二：社招 · 已有 Agent 项目，融入 RAG 深度（非实测）
...
> 💡 **使用提醒与重要说明**：
>
> **1. 关于证据边界**：可以优化表达与信息密度，但不得编造量化指标、业务规模、生产部署或用户效果。生成简历后，应能从报告、manifest、测试或运行记录追溯每一条主张。
>
