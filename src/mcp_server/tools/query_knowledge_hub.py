...
This tool uses the configured Dense baseline or optional Dense + BM25 + RRF
strategy. Results include source citations for reference.
...
class QueryKnowledgeHubTool:
    ...
    async def execute(
        ...
        trace.metadata["retrieval_strategy"] = self.settings.retrieval.strategy
        ...
        try:
            ...
            _init_t0 = _time.perf_counter()
            await asyncio.to_thread(self._ensure_initialized, effective_collection)
            _init_elapsed = (_time.perf_counter() - _init_t0) * 1000.0
            trace.record_stage("initialization", {
                "method": "cached_component_factory",
    ...
    def _close_vector_store(self) -> None:
    ...
    def close(self) -> None:
        """Release the cached Chroma client used by this tool."""
