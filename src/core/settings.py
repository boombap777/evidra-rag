...
SETTINGS_PATH_ENV = "MODULAR_RAG_SETTINGS_PATH"

RetrievalStrategy = Literal["dense", "hybrid_rrf"]
SUPPORTED_RETRIEVAL_STRATEGIES = frozenset({"dense", "hybrid_rrf"})
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        settings = cls(
            ...
            retrieval=RetrievalSettings(
                ...
                strategy=str(retrieval.get("strategy", "hybrid_rrf")).strip().lower(),  # type: ignore[arg-type]
            ),
            rerank=RerankSettings(
...
def validate_settings(settings: Settings) -> None:
    ...
    if settings.retrieval.strategy not in SUPPORTED_RETRIEVAL_STRATEGIES:
        choices = ", ".join(sorted(SUPPORTED_RETRIEVAL_STRATEGIES))
        raise SettingsError(
            f"retrieval.strategy must be one of: {choices}; "
            f"got {settings.retrieval.strategy!r}"
        )
...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    """
    configured_path = path if path is not None else os.getenv(SETTINGS_PATH_ENV)
    settings_path = Path(configured_path) if configured_path else DEFAULT_SETTINGS_PATH
