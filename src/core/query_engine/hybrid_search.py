...
class HybridSearch:
    ...
    def _extract_config(self, settings: Optional[Settings]) -> HybridSearchConfig:
        ...
        return HybridSearchConfig(
            ...
            enable_dense=True,
            enable_sparse=getattr(retrieval_config, 'strategy', 'hybrid_rrf') == 'hybrid_rrf',
            parallel_retrieval=getattr(retrieval_config, 'strategy', 'hybrid_rrf') == 'hybrid_rrf',
            metadata_filter_post=True,
            strategy=getattr(retrieval_config, 'strategy', 'hybrid_rrf'),
        )
    ...
    def search(
        ...
        if trace is not None:
            trace.metadata["retrieval_strategy"] = self.config.strategy
        ...
        if dense_error and sparse_error:
            ...
            fused_results = []
        elif self.config.enable_dense and not self.config.enable_sparse:
            # The benchmark-selected dense strategy deliberately skips BM25/RRF.
            fused_results = dense_results or []
        elif self.config.enable_sparse and not self.config.enable_dense:
            fused_results = sparse_results or []
        else:
            # Step 4: Fuse results
