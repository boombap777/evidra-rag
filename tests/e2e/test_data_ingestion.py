...
class TestIngestScriptIntegration:
    ...
    def test_creates_vector_store_data(self, tmp_path):
        ...
        # Read the selected test profile, not a stale database in the author's checkout.
        assert result.returncode == 0, "Ingestion must actually succeed"
        from src.core.settings import load_settings, resolve_path
        from src.libs.vector_store.vector_store_factory import VectorStoreFactory

        settings = load_settings()
        assert resolve_path(settings.vector_store.persist_directory).is_dir()
        store = VectorStoreFactory.create(settings, collection_name="e2e_test_persistence")
        try:
            assert store.collection.count() > 0, "The requested collection must contain ingested chunks"
        finally:
            store.close()
