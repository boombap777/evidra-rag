...
The main pipeline class runs against the deterministic offline test profile:
- rule-based chunk refinement and metadata enrichment
- optional vision captioning disabled
- local hash embeddings for contract validation
...
class TestIngestionPipeline:
    ...
    @pytest.fixture
    def settings(self):
        """Load the session's isolated offline settings profile."""
    ...
    def test_pipeline_with_complex_technical_doc(self, settings, complex_pdf_path):
        ...
        try:
            ...
            # Every chunk must be handled by the configured rule/LLM path.
...
class TestPipelineComponents:
    """Test the committed local profile and opt-in model connectivity."""
    ...
    def test_settings_loads_correctly(self, settings):
        ...
        # Local deployment keeps generation disabled for this retrieval server.
        assert settings.llm.provider == "ollama"
        assert settings.llm.model == "disabled"
        print(f"[OK] LLM: {settings.llm.provider}/{settings.llm.model}")
        
        # Embedding settings
        assert settings.embedding.provider == "openai"
        assert settings.embedding.model == "Qwen3-Embedding-0.6B-Q8_0"
        assert settings.embedding.dimensions == 1024
        print(f"[OK] Embedding: {settings.embedding.provider}/{settings.embedding.model}")
        
        assert settings.vision_llm is None or settings.vision_llm.enabled is False
        ...
        print("[OK] Ingestion LLM enhancement: disabled for local-first deployment")
    
    @pytest.mark.live
    def test_embedding_creates_vectors(self, settings):
        """Test the configured local Qwen3 embedding service."""
    ...
    def test_llm_responds(self, settings):
        """Test generation only when a local LLM is explicitly configured."""
        if settings.llm.model == "disabled":
            pytest.skip("Local deployment intentionally disables generation")
