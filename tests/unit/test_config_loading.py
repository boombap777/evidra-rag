...
def test_invalid_retrieval_strategy_raises_error(tmp_path: Path) -> None:
    source = Path("config/settings.yaml").read_text(encoding="utf-8")
    settings_path = tmp_path / "settings.yaml"
    _write_yaml(settings_path, source.replace('strategy: "dense"', 'strategy: "graph"'))

    with pytest.raises(SettingsError, match="retrieval.strategy"):
        load_settings(settings_path)
