...
class TestConfigService:
    ...
    @patch("src.observability.dashboard.services.config_service.load_settings")
    def test_retrieval_card_exposes_selected_strategy(self, mock_load) -> None:
        mock_load.return_value = _fake_settings()
        card = next(
            item for item in ConfigService().get_component_cards()
            if item.name == "Retrieval"
        )
        assert card.provider == "dense"
        assert card.extra["strategy"] == "dense"
