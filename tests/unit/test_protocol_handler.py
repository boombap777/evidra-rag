...
class TestExecuteTool:
    ...
    @pytest.mark.asyncio
    async def test_schema_validation_rejects_missing_required_parameter(
        self, protocol_handler: ProtocolHandler, sample_tool_schema: Dict[str, Any]
    ) -> None:
        ...
        result = await protocol_handler.execute_tool("search", {"top_k": 2})

        assert result.isError is True
        assert "missing required" in result.content[0].text.lower()
        handler.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_schema_validation_rejects_wrong_type_and_range(
        self, protocol_handler: ProtocolHandler
    ) -> None:
        handler = AsyncMock(return_value="unused")
        schema = {
        ...
        protocol_handler.register_tool("search", "Search", schema, handler)

        wrong_type = await protocol_handler.execute_tool(
            "search", {"query": "hello", "top_k": True}
        )
        too_large = await protocol_handler.execute_tool(
            "search", {"query": "hello", "top_k": 21}
        )
        unknown = await protocol_handler.execute_tool(
            "search", {"query": "hello", "extra": "nope"}
        )

        assert all(result.isError is True for result in (wrong_type, too_large, unknown))
        handler.assert_not_awaited()

    def test_close_calls_registered_cleanup_once_per_invocation(
        self, protocol_handler: ProtocolHandler, sample_tool_schema: Dict[str, Any]
    ) -> None:
        cleanup = MagicMock()
        protocol_handler.register_tool(
